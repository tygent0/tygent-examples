...
try:
    ...
    from tygent.integrations.google_ai import GoogleAIIntegration, GoogleAIBatchProcessor
except ImportError:
...
async def main():
    ...
    google_ai.addNode(
        ...
        dependencies=["destination_analysis", "activity_suggestions", "accommodation_suggestions"]
    )
    ...
    async def process_city(city: str, model: Any) -> Dict[str, str]:
        response = await model.generate_content(f"What are the top 3 attractions in {city}?")
        return {
            "city": city,
    ...
    print("With standard sequential processing, this would have taken significantly longer.")
