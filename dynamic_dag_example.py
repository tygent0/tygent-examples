...
async def activity_recommendations(weather: Dict[str, Any], traffic: Dict[str, Any]) -> Dict[str, Any]:
...
async def travel_planning_workflow(destination: str) -> str:
    ...
    try:
        ...
        print(f"✓ Weather: {weather_data['temperature']}°F, {weather_data['conditions']}")
        ...
        print(f"✓ Backup weather: {weather_data['temperature']}°F, {weather_data['conditions']}")
    
    # Step 2: Get traffic information (can run in parallel with weather)
    traffic_data = await traffic_api_call(destination)
    print(f"✓ Traffic: {traffic_data['traffic_level']} with {traffic_data['incidents']} incidents")
...
async def get_indoor_alternatives(location: str) -> Dict[str, Any]:
    ...
    await asyncio.sleep(0.3)
    return {
        "options": ["art galleries", "indoor markets", "historic buildings"]
    }

async def get_local_alternatives(location: str, original_recs: Dict[str, Any]) -> Dict[str, Any]:
